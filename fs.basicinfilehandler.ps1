...
if (-not (Test-Path -LiteralPath $args[0] -PathType Leaf)) {
	Write-Output "[fs.basicinfilehandler] Given <INPUT-FILE> parameter could not be found as a file" -ForegroundColor DarkRed
	exit 1
}

if (-not ((Get-Content $args[0] | Measure-Object -Line).Lines -gt 0)) {
	Write-Output "[fs.basicinfilehandler] Given <INPUT-FILE> has been found but seems to be empty" -ForegroundColor DarkRed
	exit 2
}

if (-not (Get-Command -Name $args[1] -ErrorAction Ignore)) {
	Write-Output "[fs.basicinfilehandler] Given <PROCESSING-SCRIPT> command does not exist" -ForegroundColor DarkRed
	exit 3
}
